  ...
  for(itheta=0; itheta<ntheta2; itheta++) {
    ...
    for(iphi=0; iphi<nphi; iphi++) {
      ...
      for(id=0; id<ctl->nd; id++) {
        ...
        rad=LIN(alpha[idx], obs2->rad[idx][id],
            alpha[idx+1], obs2->rad[idx+1][id], alpha2);
...
void jur_sca_srcfunc_sca_sun(ctl_t *ctl,
  ...
  if(tsurf<0) {
    ...
    for(id=0; id<ctl->nd; id++) {
      ...
      src_sca[id] += 6.764e-5 * phase2 * jur_planck(TSUN, ctl->nu[id]) * obs->tau[0][id];
    }
  }
