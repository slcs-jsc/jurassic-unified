...
int main(int argc, char *argv[]) {
  ...
  char zone[LENMAX], climpath[LENMAX];
  ...
  /* Read geolocations... */
  zmin=jur_scan_ctl(argc, argv, "ZMIN", -1, "0", NULL);
  zmax=jur_scan_ctl(argc, argv, "ZMAX", -1, "0", NULL);
  dz=jur_scan_ctl(argc, argv, "DZ", -1, "1", NULL);

  /* Set geolocations... */
  if(argv[2][0]!='-'){
    jur_read_atm(NULL, argv[2], &ctl, &atm);
  } else {
    atm.np=0;
  ...
  /* Get climate zone */
  jur_scan_ctl(argc, argv, "CLIMZONE", -1, "midl", zone);
  ...
  jur_scan_ctl(argc, argv, "CLIMPATH", -1, "", climpath);
  ...
  /* Write data to disk... */
  jur_write_atm(NULL, argv[3], &ctl, &atm);
...
void get_clim_data(int czone,
  ...
  FILE *in;
  char file[LENMAX], line[LENMAX], *tok, name[LENMAX];
...
void climatology(ctl_t *ctl,
  ...
  for(ip=0; ip<atm->np; ip++) {
    ...
    /* Get altitude index... */
    iz=jur_locate(clim->z, 121, atm->z[ip]);
