...
void changed_jur_raytrace(ctl_t *ctl,
    atm_t *atm,
    obs_t *obs,
    aero_t *aero,
    los_t *los,
...
double changed_jur_sca_refractivity(double p,
    double t);

/* Find tangent point of a given LOS. */
void changed_jur_sca_tangent_point(los_t *los,
    double *tpz,
    double *tplon,
    double *tplat);

/* Find ground or TOA intersection point of a LOS. */
void changed_jur_intersection_point(ctl_t *ctl,
    atm_t *atm,
    double *znew,
    los_t *los,
    int ip,
...
void changed_jur_add_aerosol_layers(ctl_t *ctl,
    atm_t *atm,
    los_t *los,
    aero_t *aero);
...
void changed_jur_raytrace(ctl_t *ctl,
  ...
  while(1) {
    ...
    // FIXME: we replace it with the code from jurassic version!
    ...
    /*if(z<zmin+0.001 || z>zmax+0.001) {
      ...
      los->lon[los->np]=lon;
      los->lat[los->np]=lat;
      changed_jur_intersection_point(ctl, atm, (z<zmin+0.001 ? &zmin : &zmax), los, los->np, los, los->np);
      los->ds[los->np]=0.;
    }*/
    ...
    if (z < zmin || z > zmax) {
      stop = (z < zmin ? 2 : 1);
      double frac = ((z < zmin ? zmin : zmax) - los->z[los->np - 1]) / (z - los->z[los->np - 1]);
      jur_geo2cart(los->z[los->np - 1], los->lon[los->np - 1], los->lat[los->np - 1], xh);
      for (int i = 0; i < 3; i++)
    ...
    // FIXME: if statemt has to be removed!
    // if(stop==0) {
    ...
    if(ctl->refrac && z<=zrefrac)
      n=1+changed_jur_sca_refractivity(p, t);
    else
      n=1;
    ...
    if(ctl->refrac && z<=zrefrac) {
      ...
      jur_intpol_atm_geo(ctl, atm, z, lon, lat, &p, &t, q, k);
      n=changed_jur_sca_refractivity(p, t);
      for(i=0; i<3; i++) {
        xh[i]+=h;
        jur_cart2geo(xh, &z, &lon, &lat);
        jur_intpol_atm_geo(ctl, atm, z, lon, lat, &p, &t, q, k);
        naux=changed_jur_sca_refractivity(p, t);
        ng[i]=(naux-n)/h;
        xh[i]-=h;
      }
    } else
  ...
  // FIXME: it is removed, because it is not part of jurassic
  ...
  changed_jur_sca_tangent_point(los, &obs->tpz[ir], &obs->tplon[ir], &obs->tplat[ir]);
  ...
  if(!ignore_scattering) {
    if (ctl->sca_n > 0)
      changed_jur_add_aerosol_layers(ctl,atm,los,aero);
  }
}
    ...
    if ( (los->z[ip-1] < altimax || los->z[ip] < altimax) &&
      ...
      for (il=0; il<jl;il++){ /* loop over cloud edges */
        ...
        if(los->z[ip-1] > alti[il] && los->z[ip] < alti[il]){
          changed_jur_intersection_point(ctl, atm, &alti[il], los, ip, los_aero, los_aero->np);
          los_aero->np++; 
        }
        /* von unten */
        if(los->z[ip-1] < alti[jl-il-1] && los->z[ip] > alti[jl-il-1]){
          changed_jur_intersection_point(ctl, atm, &alti[jl-il-1], los, ip, los_aero, los_aero->np);
          los_aero->np++;
        }
      }
    }
