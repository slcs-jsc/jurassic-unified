...
//     printf("ray #%i point#%i %g %g %g\n", ir, np, lon, lat, z); // to debug the jur_raytracer
  ...
  void pos_scatter_jur_intersection_point(ctl_t *ctl,
    ...
    intpol_atm_geo_pt(ctl, atm, atmIdx, atmNp, los_aero[jp].z, los_aero[jp].lon, los_aero[jp].lat, &los_aero[jp].p, &los_aero[jp].t);		// Interpolate atmospheric data
    intpol_atm_geo_qk(ctl, atm, atmIdx, atmNp, los_aero[jp].z, los_aero[jp].lon, los_aero[jp].lat, los_aero[jp].q, los_aero[jp].k);			// Interpolate atmospheric data
  }
